...
export const placeOrder: RequestHandler = async (req, res) => {
  try {
    ...
    if (orderData.side === 'buy') {
      const estimatedCost = (orderData.price || getCurrentPrice(orderData.symbol)) * orderData.quantity;
      if (estimatedCost > accountBalance * 0.95) { // 95% to account for fees
    ...
    res.json({
      ...
      data: {
        ...
        status: order.status,
        executed,
        message: executed ? 'Order executed successfully' : 'Order placed successfully',
      },
    });
  } catch (error) {
    res.status(400).json({
...
export const getOrders: RequestHandler = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: userOrders.sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime()),
    });
  } catch (error) {
    res.status(500).json({
      success: false,
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        pendingOrders: orders.filter(o => o.userId === userId && o.status === 'pending').length,
      },
    });
  } catch (error) {
    res.status(500).json({
